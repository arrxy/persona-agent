...
export const env = {
  ...
  CHUNK_TARGET_WORDS: Number(process.env.CHUNK_TARGET_WORDS) || 300,
  CHAT_MODEL: process.env.CHAT_MODEL ?? "gpt-4o-mini",
  CHAT_MAX_CONTEXT_TOKENS: Number(process.env.CHAT_MAX_CONTEXT_TOKENS) || 10_000,
  CHAT_SESSION_TURNS: Number(process.env.CHAT_SESSION_TURNS) || 6,
  CREATOR_RAG_TOP_K: Number(process.env.CREATOR_RAG_TOP_K) || 12,
  CREATOR_RAG_MIN_SCORE: Number(process.env.CREATOR_RAG_MIN_SCORE) || 0.72,
  TRANSCRIPT_CANDIDATE_LIMIT:
    Number(process.env.TRANSCRIPT_CANDIDATE_LIMIT) || 60,
  PRESENCE_THRESHOLD: Number(process.env.PRESENCE_THRESHOLD) || 0.35,
  FALLBACK_SELECT_LIMIT: Number(process.env.FALLBACK_SELECT_LIMIT) || 15,
  DEFAULT_SOURCE_VIDEO_LIMIT:
    Number(process.env.DEFAULT_SOURCE_VIDEO_LIMIT) || 150,
  TRANSCRIPT_FETCH_DELAY_MS:
    Number(process.env.TRANSCRIPT_FETCH_DELAY_MS) || 500,
